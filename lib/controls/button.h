...
class Button {
 public:
  ...
  const bool IsPressed() { return is_pressed_; }

  const bool IsUp() { return !IsPressed(); }
  ...
  const int GetFramesHeld() { return frames_held_; }
  ...
  const int GetFramesReleased() { return frames_released_; }
