...
class Vector3D {
 public:
  ...
  Vector3D GetAbs() const { return {abs(x), abs(y), abs(z)}; }

  // Not sure how well this would work for non-floats, meh whatever
  static Vector3D RandomVector(const T max_magnitude) {
    ...
    do {
      ...
      float_vec.z = random::Float(neg_magnitude, max_magnitude);
             ...
             hypotenuese_squared);  // Repeat until we're within the unit sphere
