 ...
 public:
  ...
  void blendPixel(int x, int y, CRGB* blendColor, float blendFactor) {
    if (y >= 0 && y < lengthStrips) {
      ...
      int red = strips[x][y].red * (1 - blendFactor) +
                blendColor->red * (blendFactor);
      int green = strips[x][y].green * (1 - blendFactor) +
                  blendColor->green * (blendFactor);
      int blue = strips[x][y].blue * (1 - blendFactor) +
                 blendColor->blue * (blendFactor);
